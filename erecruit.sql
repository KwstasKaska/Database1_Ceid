...
recr_username VARCHAR(12) NOT NULL,
date_hour DATETIME DEFAULT NOW(),
duration TIME NOT NULL,
sxolia TEXT NOT NULL,
score_personality ENUM('1','2','3','4','5'),
cand_education ENUM('1','2','3','4','5'),
cand_experience ENUM('1','2','3','4','5'),
PRIMAREY KEY(cand_username,recr_username),
CONSTRAIT INTERVIEW_CAND FOREIGN KEY cand_username REFERENCES candidate(username) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAIT INTERVIEW_RECR FOREIGN KEY recr_username REFERENCES recruiter(username) ON DELETE CASCADE ON UPDATE CASCADE
);
...
job_id INT(4) NOT NULL,
PRIMAREY KEY(recruiter_username,job_id),
CONSTRAIT ANNOUNC FOREIGN KEY recruiter_username REFERENCES recruiter(username) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAIT RECV FOREIGN KEY job_id REFERENCES job(id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE tomeis(
title VARCHAR(12) NOT NULL,
text TEXT,
tomeas_belongs_to VARCHAR(36),
PRIMAREY KEY(title),
CONSTRAINT TOMEAS FOREIGN KEY tomeas_belongs_to REFERENCES tomeis(title) ON DELETE CASCADE ON UPDATE CASCADE
);


CREATE TABLE computer_systems(
title_1 VARCHAR(12) NOT NULL,
PRIMAREY KEY(title_1),
CONSTRAINT TOMEAS1 FOREIGN KEY title_1 REFERENCES tomeis(title) ON DELETE CASCADE ON UPDATE CASCADE
);


CREATE TABLE computer_networks(
title_2 VARCHAR(12) NOT NULL,
PRIMAREY KEY(title_2),
CONSTRAINT TOMEAS1 FOREIGN KEY title_2 REFERENCES tomeis(title) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE drasthriopoieitai(
etaireia_AFM CHAR(9) NOT NULL,
tomeis_title VARCHAR(12) NOT NULL,
PRIMAREY KEY(etaireia_AFM,tomeis_title),
CONSTRAINT AFMTITLE1 FOREIGN KEY etaireia_AFM REFERENCES etaireia(AFM) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT AFMTITLE2 FOREIGN KEY tomeis_title REFERENCES tomeis(title) ON DELETE CASCADE ON UPDATE CASCADE
);
 
CREATE TABLE katagrafh(
username VARCHAR(12) NOT NULL,
hour_date DATETIME,
success ENUM('YES','NO'),
eidos ENUM('INSERT','UPDATE','DELETE'),
table_name ENUM();
PRIMAREY KEY (username),
CONSTRAINT ABC FOREIGN KEY username REFERENCES user(username) ON DELETE CASCADE ON UPDATE CASCADE
);